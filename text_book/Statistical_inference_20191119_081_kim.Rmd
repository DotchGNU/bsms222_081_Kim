...
take_sample <- function(n, p){
  x <- sample(c(1,-3), n, replace=TRUE, prob=c(p,1-p))
  sum(x)
}
games <- replicate(B, take_sample(2000, 0.6))
```
...
$$
Pr(\bar{X} - 1.96\hat{SE}(\bar{X}) <= p <= \bar{X} + 1.96\hat{SE}(\bar{X}))
$$

By subtracting and dividing the same quantities in all parts of the equation, we get that the above in equivalent to:

$$
Pr(-1.96 <= (\bar{X}-p)/\hat{SE}(\bar{X})<= 1.96)
$$
...
Spectifically, we will use all the national polls that ended within one week before the election.
...
1. For the first poll, you can obtain the sample size and estimated Clinton percentage with :
...
poll <- polls %>% select("pollster", 'enddate', 'x_hat', 'lower', 'upper')
poll
```
...
```{r}
p <- 0.482
poll <- poll %>% mutate(hit=ifelse(upper > p & lower < p, 1, 0))
poll
```
...
N <- polls$samplesize[1]
x_hat <- polls$rawpoll_clinton[1]/100
temp <- polls$d_hat[1] + c(-1.96, 1.96) * 2 * sqrt(x_hat * (1 - x_hat) / N)
temp[2] - temp[1]
```
...
$$
\sqrt{N}(\bar{X}-0.5)/\sqrt{0.5(1-0.5)}
$$
is standard normal. We there fore can compute the probability above, which is the p-value.

$$
Pr(\sqrt{N}(\bar{X}-0.5)/\sqrt{0.5(1-0.5)}>\sqrt{N}(0.02)/\sqrt{0.5(1-0.5)})
$$
...
## 15.10.1 Lady Testing Tea (Fisher's exact test)
...
Inthis cases, the odds ratio is $\frac{a/c}{b/d}$ when is equivalent to $(ad)/(bc)$
...
```{r}
two_by_two <- tibble(record = c('wins', 'losses'),
                         important_events = c(8, 9), 
                         normal_events = c(350-8, 150-9))
two_by_two %>% select(-record) %>% chisq.test() %>% .$p.value
```
...
two_by_two %>% select(-record) %>%
  mutate(important_events = important_events*2, 
         normal_events = normal_events*2) %>%
  chisq.test() %>% .$p.value
```
