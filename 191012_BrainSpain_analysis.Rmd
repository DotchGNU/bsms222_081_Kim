...
MA_miR_age_group <- miR_DFC_DE %>%
  ...
  geom_hline(yintercept = -1, colour="darkblue", linetype="dashed", alpha=0.5) + 
  geom_vline(xintercept = 4, colour="darkgrey", linetype="dashed") +
  geom_text(aes(x=11, y=4, label="FE>1, Average expression>4"), size=2.5, color="darkred") +
  geom_text(aes(x=11, y=-4, label="FE<1, Average expression>4"), size=2.5, color="darkblue") +
...
VOL_miR_age_group <- miR_DFC_DE %>%
  ...
  coord_cartesian(xlim =c(-5, 5), ylim = c(0, 4)) +
   geom_text(aes(x=4, y=1.5, label="p-value<0.05"), size=3, color="darkgrey") +
...
MA_adult <- adult_miR %>% ggplot(aes(age_group, Fold_Enrichment, group=miRNA, color=Gender_specific_by_volume)) +
                geom_line()+
                geom_text_repel(aes(label=ifelse(age_group=="Adult",miRNA,"")), size=2.5, nudge_x=-0.01, nudge_y=0.01, force=25)+
                geom_point(size=3.5)+
...
MA_youth <- youth_miR %>% ggplot(aes(age_group, Fold_Enrichment, group=miRNA, color=Gender_specific_by_volume)) +
                geom_line()+
                geom_text_repel(aes(label=ifelse(age_group=="Youth",miRNA,"")), size=2.5, nudge_x=-0.01, force=30)+
                geom_point(size=3.5)+
                scale_x_discrete(limits=c("Youth", "Adult")) +
                coord_cartesian(ylim = c(-3, 3)) +
                xlab("") +
...
CUM_youth <- ggplot() +
  ...
  geom_text(aes(x=-1, y=1, label = "Target of female specific miRNA in DFC"), color = "red", hjust=0, size=2.5) + 
  geom_text(aes(x=-1, y=0.92, label = "Target of male specific miRNA in DFC"), color = "blue", hjust=0, size=2.5) +
  geom_text(aes(x=-1, y=0.84, label = "Total mRNA in DFC"), color = "Darkgrey", hjust=0, size=2.5) + 
  xlim(c(-1,1)) +
  ylim(c(0,1))+
