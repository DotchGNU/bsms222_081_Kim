...
### 3.2.4 DFC region만 분리하여 분석 진행할 dataset 만들기 : `miRDB_DFC`
...
* miRNA Fold_Enrichment top 2개까지 후보로 삼아 target repression 정도를 cumulative graph로 확인 

### 4.1.1 Adult female miRNA target : `adult_FmiR`

```{r}
#adult_miR : adult에서 specific하게 gender specificity가 있는 miRNA group 
adult_miR %>% filter(age_group=="Adult") %>% arrange(desc(Fold_Enrichment)) %>% head(n=2) %>% .$miRNA 
```
...
```{r}
adult_FmiR_target <- read_target("./adult_female")
```

### 4.1.2 Adult male miRNA : `adult_MmiR`

```{r}
adult_miR %>% filter(age_group=="Adult") %>% arrange(Fold_Enrichment) %>% head(n=2) %>% .$miRNA 
```

```{r}
adult_MmiR_target <- read_target("./adult_male") 
```
...
```{r}
youth_Fmir_target <- read_target("./youth_female")
```
...
```{r}
youth_Mmir_target <- read_target("./youth_male")
```
...
CUM_youth <- ggplot() +
  ...
  ggtitle("Youth-specific sexual differenctial miRNA") +
  theme_bw()
...
CUM_adult <- ggplot() +
  ...
  xlim(c(-1,1)) +
  ylim(c(0,1))+
  annotate("rect", alpha = 0.5, ymin=0,ymax=6,xmin=0,xmax=25,fill="brown1") +
  annotate("rect", alpha = 0.5, ymin=0,ymax=6,xmin=0,xmax=-25,fill="blue") +
  xlab("Fold Enrichment [log2(Female/Male)]")+
  ylab("Cumulative fraction")+
  geom_vline(xintercept = 0, colour="darkgrey", linetype="dashed") + 
  ggtitle("Adult-specific sexual differenctial miRNA") +
  theme_bw()
```

```{r}
ggarange(SLOPE_age, ggarrange(CUM_youth,CUM_adult), nrow=2)
```
